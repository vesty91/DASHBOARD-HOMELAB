...
Loopback, unspecified, link-local/metadata et multicast sont bloqués, y compris via DNS et via leurs formes IPv4-mapped IPv6. Les littéraux IPv6 sont normalisés sans leurs crochets URL. Les plages privées IPv4, CGNAT et ULA IPv6 restent autorisées pour le homelab, **sauf** les endpoints metadata cloud explicitement dénylistés (`100.100.100.200`, `fd00:ec2::254` et variantes textuelles équivalentes). Les redirects ne sont jamais suivis, les headers restent anonymes, le body est détruit dès réception du status, et un même timeout de 500 à 10 000 ms borne la résolution DNS puis la requête avec le budget restant.
