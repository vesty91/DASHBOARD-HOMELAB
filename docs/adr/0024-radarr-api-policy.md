# ADR 0024 — Radarr API policy

## Statut

Accepté pour la Phase 18.5 (Radarr).

## Contexte

Radarr expose une API HTTP officielle v3. Un proxy générique, une clé
`apikey` en query string ou un chemin hors allowlist sont interdits. Grab/remove
de file et édition de films restent hors scope. Les commandes ciblées
`RefreshMovie` et `MoviesSearch` (un `movieId`) sont autorisées en Phase 21.5.

Officiel : https://radarr.video/docs/api/

## Décisions

### 1. Package `@dashboard/radarr`
...
- `POST /api/v3/command` (Phase 21.5) avec corps allowlisté
  `{ name: "RefreshMovie", movieIds: [id] }` ou
...
Interdit : PUT/DELETE, GET `/api/v3/command`, queue grab/remove, rename, delete,
...
Jamais query string, même si Radarr l'accepte.
...
- `queue/status` : compteurs numériques `totalCount` / `count` /
  `unknownCount` uniquement s'ils sont des entiers sûrs. Les booléens
  officiels `errors`/`warnings` sont ignorés. Pas de records ni titres.
- `movie` : `{ count, truncated }`. Cap 2000. Jamais titres, chemins, dossiers.
...
`radarr.read` en conjonction de `integration.use|manage`. `radarr.command` en
...
`radarr-overview` : `publicSafe=false`. Capabilities : `status.read`,
`command.queue`.
...
- `POST /api/v3/command` avec `RefreshMovie` + un `movieId`
- `POST /api/v3/command` avec `MoviesSearch` + un `movieId`

Toujours interdit : rename, delete, collections, settings, commandes sans ID,
GET command, query, `apikey` en query.
