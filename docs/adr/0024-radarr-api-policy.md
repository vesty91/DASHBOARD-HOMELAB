...
Radarr expose une API HTTP officielle v3. Les mutations (commandes, grab/remove
de file, édition de films) sont hors scope. Un proxy générique, une clé
...
`radarr.read` en conjonction de `integration.use|manage`. ADMIN par défaut ne
l'obtient pas. SYSTEM_ADMIN via le catalogue `PERMISSIONS`. Widget
`radarr-overview` : `publicSafe=false`. Capability : `status.read`.
