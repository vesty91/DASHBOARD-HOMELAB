export interface ImmichRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const IMMICH_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryImmichRefreshFence implements ImmichRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = IMMICH_REFRESH_FENCE_MAX_ENTRIES) {
