...
export const grafanaServiceAccountTokenSchema = z
  .string()
  .min(1)
  .max(512)
  .refine(
    (value) => !/[^\u0021-\u007E]/u.test(value),
    "Grafana service account token must be visible ASCII",
  );
...
export const grafanaSecretSchema = z.object({
  serviceAccountToken: grafanaServiceAccountTokenSchema,
});

export const grafanaIntegrationInputSchema = z.object({
  integrationId: z.uuid(),
});

export type GrafanaConfig = z.infer<typeof grafanaConfigSchema>;
export type GrafanaSecrets = z.infer<typeof grafanaSecretSchema>;
