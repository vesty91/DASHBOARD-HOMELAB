export interface GrafanaRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const GRAFANA_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryGrafanaRefreshFence implements GrafanaRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = GRAFANA_REFRESH_FENCE_MAX_ENTRIES) {
