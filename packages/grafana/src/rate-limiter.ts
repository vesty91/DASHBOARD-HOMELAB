...
export const GRAFANA_REFRESH_RATE_LIMIT = 10;
export const GRAFANA_REFRESH_RATE_WINDOW_MS = 60_000;
export const GRAFANA_REFRESH_MAX_TRACKED_KEYS = 10_000;

class MemoryGrafanaRateLimiter implements IntegrationRateLimiter {
...
export class MemoryGrafanaRefreshRateLimiter extends MemoryGrafanaRateLimiter {
  constructor(
    limit = GRAFANA_REFRESH_RATE_LIMIT,
    windowMs = GRAFANA_REFRESH_RATE_WINDOW_MS,
    now: () => number = () => Date.now(),
    maxTrackedKeys = GRAFANA_REFRESH_MAX_TRACKED_KEYS,
  ) {
