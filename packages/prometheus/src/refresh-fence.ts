export interface PrometheusRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const PROMETHEUS_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryPrometheusRefreshFence implements PrometheusRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = PROMETHEUS_REFRESH_FENCE_MAX_ENTRIES) {
