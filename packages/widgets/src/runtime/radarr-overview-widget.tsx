...
export function RadarrOverviewWidget({ view }: { view: RadarrOverviewView | undefined }) {
  ...
  if (view.status === "configuration-missing")
    return <p role="status">Sélectionnez une intégration Radarr</p>;
  if (view.status === "empty") return <p role="status">Intégration Radarr introuvable</p>;
  if (view.status === "error") return <p role="status">Ce widget a rencontré une erreur</p>;
  return (
    <div className="widget-radarr-overview">
      <p>{versionLabel(view.version)}</p>
      <p className="widget-state">
        {view.movieCount === null ? "Films indisponibles" : `${view.movieCount} films`}
