export interface UptimeKumaRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const UPTIME_KUMA_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryUptimeKumaRefreshFence implements UptimeKumaRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = UPTIME_KUMA_REFRESH_FENCE_MAX_ENTRIES) {
