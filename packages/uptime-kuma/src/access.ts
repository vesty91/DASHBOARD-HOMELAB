...
import type { UptimeKumaPermissionsView } from "./types";
...
export function uptimeKumaPermissionsView(actor: IntegrationActor): UptimeKumaPermissionsView {
  ...
  return {
    canRead: integrationUse && hasAny(actor, ["uptime-kuma.read"]),
    canManage: hasAny(actor, ["integration.manage"]),
  };
}

export type UptimeKumaAccessKind = "read" | "manage";

export function assertUptimeKumaAccess(actor: IntegrationActor, kind: UptimeKumaAccessKind): void {
  if (!isActive(actor)) throw new IntegrationError("UNAUTHORIZED", "Authentication required");
  const view = uptimeKumaPermissionsView(actor);
  switch (kind) {
    case "read":
