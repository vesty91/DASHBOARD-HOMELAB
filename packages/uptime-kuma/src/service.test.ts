...
import { uptimeKumaIntegrationDefinition } from "./definition";
import { MemoryUptimeKumaOverviewCoalescer } from "./overview-coalescer";
import { MemoryUptimeKumaRefreshRateLimiter } from "./rate-limiter";
import { MemoryUptimeKumaRefreshFence } from "./refresh-fence";
import { createUptimeKumaService } from "./service";
...
function text(body: string, status = 200): SecureHttpResult {
  return { ok: true, status, body: Buffer.from(body), latencyMs: 4 };
}

function official(): SecureHttpResult {
  return text('monitor_status{monitor_id="1",monitor_name="Web"} 1');
}
...
function serviceWith(
  ...
  extras: {
    ...
    fence?: MemoryUptimeKumaRefreshFence;
    coalescer?: MemoryUptimeKumaOverviewCoalescer;
    limiter?: MemoryUptimeKumaRefreshRateLimiter;
  } = {},
) {
  return createUptimeKumaService({
    store: store.store,
    registry: createIntegrationRegistry().register(uptimeKumaIntegrationDefinition),
    cache: extras.cache ?? new MemoryIntegrationCache(),
    request,
    refreshRateLimiter: extras.limiter ?? new MemoryUptimeKumaRefreshRateLimiter(),
    refreshFence: extras.fence ?? new MemoryUptimeKumaRefreshFence(),
    overviewCoalescer: extras.coalescer ?? new MemoryUptimeKumaOverviewCoalescer(),
    keyring: store.keyring,
  });
}

describe("createUptimeKumaService", () => {
  it("returns a sanitized overview for a reader", async () => {
    ...
    expect(overview.monitors.data?.monitors[0]?.name).toBe("Web");
    expect(JSON.stringify(overview)).not.toContain(API_KEY);
  });

  it("requires integration.use and uptime-kuma.read together", async () => {
    const service = serviceWith(async () => official());
    await expect(
      service.getOverview(INTEGRATION_ID, actor(["uptime-kuma.read"])),
  ...
  it("redacts the API key from unauthorized errors and caches the failure", async () => {
    ...
    const service = serviceWith(async () => {
      calls += 1;
      return text(`denied ${API_KEY}`, 401);
    });
  ...
  it("coalesces concurrent overview reads", async () => {
    let calls = 0;
    const coalescer = new MemoryUptimeKumaOverviewCoalescer();
    const cache = new MemoryIntegrationCache();
    const fence = new MemoryUptimeKumaRefreshFence();
  ...
  it("rate limits refresh", async () => {
    const limiter = new MemoryUptimeKumaRefreshRateLimiter(1, 60_000, () => 1_000);
    const store = createMemoryStore();
    const service = createUptimeKumaService({
      store: store.store,
      registry: createIntegrationRegistry().register(uptimeKumaIntegrationDefinition),
      cache: new MemoryIntegrationCache(),
      request: async () => official(),
      refreshRateLimiter: limiter,
      refreshFence: new MemoryUptimeKumaRefreshFence(),
      overviewCoalescer: new MemoryUptimeKumaOverviewCoalescer(),
      keyring: store.keyring,
    });
    await service.refreshOverview(INTEGRATION_ID, systemAdmin);
  ...
  it("lists Uptime Kuma integrations beyond the first store page", async () => {
