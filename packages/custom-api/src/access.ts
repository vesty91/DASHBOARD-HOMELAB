...
import type { CustomApiPermissionsView } from "./types";
...
export function customApiPermissionsView(actor: IntegrationActor): CustomApiPermissionsView {
  ...
  return {
    canRead: integrationUse && hasAny(actor, ["custom-api.read"]),
    canManage: hasAny(actor, ["integration.manage"]),
  };
}

export type CustomApiAccessKind = "read" | "manage";

export function assertCustomApiAccess(actor: IntegrationActor, kind: CustomApiAccessKind): void {
  if (!isActive(actor)) throw new IntegrationError("UNAUTHORIZED", "Authentication required");
  const view = customApiPermissionsView(actor);
  switch (kind) {
    case "read":
