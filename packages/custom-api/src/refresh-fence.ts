export interface CustomApiRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const CUSTOM_API_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryCustomApiRefreshFence implements CustomApiRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = CUSTOM_API_REFRESH_FENCE_MAX_ENTRIES) {
