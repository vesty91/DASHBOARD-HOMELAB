export interface QbittorrentRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const QBITTORRENT_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryQbittorrentRefreshFence implements QbittorrentRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = QBITTORRENT_REFRESH_FENCE_MAX_ENTRIES) {
