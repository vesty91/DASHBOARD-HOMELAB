export interface BeszelRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const BESZEL_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryBeszelRefreshFence implements BeszelRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = BESZEL_REFRESH_FENCE_MAX_ENTRIES) {
