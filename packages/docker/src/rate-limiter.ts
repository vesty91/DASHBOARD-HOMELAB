...
export const DOCKER_ACTION_RATE_LIMIT = 10;
export const DOCKER_ACTION_RATE_WINDOW_MS = 60_000;
export const DOCKER_ACTION_MAX_TRACKED_KEYS = 10_000;

export class MemoryDockerActionRateLimiter implements IntegrationRateLimiter {
  readonly #hits = new Map<string, number[]>();
  readonly #maxTrackedKeys: number;

  constructor(
    readonly limit = DOCKER_ACTION_RATE_LIMIT,
    readonly windowMs = DOCKER_ACTION_RATE_WINDOW_MS,
    private readonly now: () => number = () => Date.now(),
    maxTrackedKeys = DOCKER_ACTION_MAX_TRACKED_KEYS,
  ) {
