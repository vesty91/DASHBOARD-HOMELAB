export interface ProwlarrRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const PROWLARR_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryProwlarrRefreshFence implements ProwlarrRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = PROWLARR_REFRESH_FENCE_MAX_ENTRIES) {
