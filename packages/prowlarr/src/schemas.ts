...
export const prowlarrApiKeySchema = z
  .string()
  .min(1)
  .max(512)
  .refine((value) => !/[^\u0021-\u007E]/u.test(value), "Prowlarr API key must be visible ASCII");

export const prowlarrConfigSchema = z
  .object({
...
export const prowlarrSecretSchema = z.object({
  apiKey: prowlarrApiKeySchema,
});

export const prowlarrIntegrationInputSchema = z.object({
  integrationId: z.uuid(),
});

export type ProwlarrConfig = z.infer<typeof prowlarrConfigSchema>;
export type ProwlarrSecrets = z.infer<typeof prowlarrSecretSchema>;
