...
import { prowlarrOverviewCacheOperation, overviewFailureCacheOperation } from "./cache-key";
import {
  PROWLARR_OVERVIEW_FAILURE_TTL_MS,
  fetchProwlarrOverview,
  prowlarrContextFromIntegration,
  overviewCacheTtl,
  type ProwlarrClientContext,
} from "./client";
import { PROWLARR_INTEGRATION_ID } from "./definition";
import { ProwlarrError, toIntegrationError } from "./errors";
import type { ProwlarrOverviewCoalescer } from "./overview-coalescer";
import type { ProwlarrRefreshFence } from "./refresh-fence";
...
export interface ProwlarrServiceDeps {
  store: IntegrationStore;
  registry: IntegrationRegistry;
  cache: IntegrationCache;
  request: (options: SecureHttpRequest) => Promise<SecureHttpResult>;
  refreshRateLimiter: IntegrationRateLimiter;
  refreshFence: ProwlarrRefreshFence;
  overviewCoalescer: ProwlarrOverviewCoalescer;
...
type CachedProwlarrOverviewFailure = Readonly<{
...
function asCachedOverview(value: unknown): ProwlarrOverview | undefined {
  if (!value || typeof value !== "object" || !("fetchedAt" in value)) return undefined;
  return value as ProwlarrOverview;
}

function asCachedOverviewFailure(value: unknown): CachedProwlarrOverviewFailure | undefined {
...
function throwCachedFailure(failure: CachedProwlarrOverviewFailure): never {
...
async function listProwlarrRecords(store: IntegrationStore): Promise<IntegrationRecord[]> {
  const found: IntegrationRecord[] = [];
  let cursor: string | undefined;
  for (let page = 0; page < LIST_MAX_PAGES; page += 1) {
    const records = await store.list(LIST_PAGE_SIZE, cursor);
    for (const record of records) if (record.type === PROWLARR_INTEGRATION_ID) found.push(record);
...
export function createProwlarrService(deps: ProwlarrServiceDeps) {
  function definition(): IntegrationDefinition<ProwlarrConfig, ProwlarrSecrets> {
    const registered = deps.registry.get(PROWLARR_INTEGRATION_ID);
    if (!registered)
      throw new IntegrationError("MISCONFIGURED", "Prowlarr definition is not registered");
    return registered as IntegrationDefinition<ProwlarrConfig, ProwlarrSecrets>;
  }

  async function requireProwlarrRecord(integrationId: string): Promise<IntegrationRecord> {
    const record = await deps.store.findById(integrationId);
    if (!record || record.type !== PROWLARR_INTEGRATION_ID)
      throw new IntegrationError("NOT_FOUND", "Définition Prowlarr introuvable");
    return record;
  }
  ...
  async function loadContext(
    ...
    const record = await requireProwlarrRecord(integrationId);
    if (!record.enabled)
      throw new IntegrationError("MISCONFIGURED", "Prowlarr integration is disabled");
    const prowlarrDefinition = definition();
    const parsed = prowlarrDefinition.configSchema.safeParse(record.config);
    if (!parsed.success)
      throw new IntegrationError("MISCONFIGURED", "Invalid Prowlarr configuration");
    requireCapability(prowlarrDefinition.capabilities, capability);
    ...
    return {
      recordId: record.id,
      secrets,
      cacheOperation: prowlarrOverviewCacheOperation(
        record.configRevision,
        encryptedSecrets,
        refreshGeneration,
      ),
      ctx: prowlarrContextFromIntegration({
        ...
        request: (options) =>
          deps.request({
            ...
            allowedSchemes: options.allowedSchemes ?? prowlarrDefinition.allowedSchemes,
            maxRetries: 0,
            maxRedirects: 0,
  ...
  async function overviewFor(
    ...
    return deps.overviewCoalescer.run(`${loaded.recordId}:${loaded.cacheOperation}`, async () => {
      ...
      try {
        const overview = await fetchProwlarrOverview(loaded.ctx);
        const frozen = Object.freeze({
          status: overview.status,
          fetchedAt: overview.fetchedAt,
          system: Object.freeze(overview.system),
          health: Object.freeze(overview.health),
          indexer: Object.freeze(overview.indexer),
          indexerStatus: Object.freeze(overview.indexerStatus),
        });
  ...
  return {
    permissions(actor: ProwlarrActor): ProwlarrPermissionsView {
      return prowlarrPermissionsView(actor);
    },
    async listIntegrations(actor: ProwlarrActor): Promise<readonly ProwlarrIntegrationMetadata[]> {
      assertProwlarrAccess(actor, "read");
      const records = await listProwlarrRecords(deps.store);
    ...
    async getIntegrationMetadata(
      integrationId: string,
      actor: ProwlarrActor,
    ): Promise<ProwlarrIntegrationMetadata> {
      assertProwlarrAccess(actor, "read");
      const record = await requireProwlarrRecord(integrationId);
    ...
    async getOverview(integrationId: string, actor: ProwlarrActor): Promise<ProwlarrOverview> {
      assertProwlarrAccess(actor, "read");
      const record = await requireProwlarrRecord(integrationId);
      return overviewFor(record.id, deps.refreshFence.current(record.id));
    },
    async refreshOverview(integrationId: string, actor: ProwlarrActor): Promise<ProwlarrOverview> {
      assertProwlarrAccess(actor, "read");
      const record = await requireProwlarrRecord(integrationId);
      const recordId = record.id;
      if (!deps.refreshRateLimiter.tryConsume(actor.userId ?? "anonymous", recordId))
        throw new IntegrationError("RATE_LIMITED", "Too many Prowlarr refreshes");
...
export type ProwlarrService = ReturnType<typeof createProwlarrService>;
