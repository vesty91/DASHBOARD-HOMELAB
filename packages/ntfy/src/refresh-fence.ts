export interface NtfyRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const NTFY_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemoryNtfyRefreshFence implements NtfyRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = NTFY_REFRESH_FENCE_MAX_ENTRIES) {
