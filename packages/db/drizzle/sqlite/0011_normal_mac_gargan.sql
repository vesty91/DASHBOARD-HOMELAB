...
CREATE UNIQUE INDEX `service_reliability_daily_service_date_uq` ON `service_reliability_daily` (`service_key`,`date_utc`);--> statement-breakpoint
CREATE INDEX `service_reliability_daily_date_idx` ON `service_reliability_daily` (`date_utc`);--> statement-breakpoint
UPDATE `server_settings` SET `schema_version` = 11, `updated_at` = unixepoch()*1000 WHERE `id` = 'global';
