...
CREATE UNIQUE INDEX `service_reliability_hourly_service_hour_uq` ON `service_reliability_hourly` (`service_key`,`hour_utc`);--> statement-breakpoint
CREATE INDEX `service_reliability_hourly_hour_idx` ON `service_reliability_hourly` (`hour_utc`);--> statement-breakpoint
UPDATE `server_settings` SET `schema_version` = 13, `updated_at` = unixepoch()*1000 WHERE `id` = 'global';