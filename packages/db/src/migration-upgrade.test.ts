...
const phase26ReliabilityMigration = new URL(
  "../drizzle/sqlite/0011_normal_mac_gargan.sql",
  import.meta.url,
);
const phase26SloMigration = new URL(
...
const phase28DependencyMigration = new URL(
  "../drizzle/sqlite/0015_great_wendell_rand.sql",
  import.meta.url,
);
...
describe("Phase 26 reliability migration", () => {
  it("adds service_reliability_daily and bumps schema_version to 11", async () => {
    const database = new DatabaseSync(":memory:");
    database.exec("PRAGMA foreign_keys=ON");
    try {
      database.exec(await readFile(phase2Migration, "utf8"));
      ...
      expect(
        database
          .prepare(
            "SELECT count(*) count FROM sqlite_master WHERE type='table' AND name='service_reliability_daily'",
          )
          .get()?.count,
      ).toBe(1);
    } finally {
  ...
  it("adds service_slos and bumps schema_version to 12", async () => {
    const database = new DatabaseSync(":memory:");
    database.exec("PRAGMA foreign_keys=ON");
    try {
      database.exec(await readFile(phase2Migration, "utf8"));
  ...
  it("adds service_reliability_hourly and bumps schema_version to 13", async () => {
    ...
    try {
      ...
      executeSqliteMigration(database, await readFile(phase28DependencyMigration, "utf8"));
      expect(
        database.prepare("SELECT schema_version FROM server_settings WHERE id='global'").get(),
      ...
      expect(
        database
          .prepare(
            "SELECT count(*) count FROM sqlite_master WHERE type='table' AND name='service_dependencies'",
          )
          .get()?.count,
      ).toBe(1);
    } finally {
