...
describe.skipIf(!connectionString)("PostgreSQL database foundation", () => {
  ...
  it("migrates integration config_revision and enforces encrypted secret CAS", async () => {
    ...
    try {
      ...
      expect(await store.deleteSecret(created.id, "missing-key")).toBe(false);
      ...
      expect(await store.persistConnectionResult(created.id, 5, "available")).toBe(true);
  ...
  it("upgrades schema 7 to 8 without losing automation rules", async () => {
    const client = createPostgresqlClient(connectionString!);
    try {
      await client.pool.query("drop schema public cascade; create schema public");
      const files = [
      ...
      await executePostgresqlMigration(
        client.pool,
        await readFile(
          new URL("../drizzle/postgresql/0008_common_post.sql", import.meta.url),
          "utf8",
        ),
      );
      expect(
      ...
      expect(
        await client.pool.query(
          "select count(*)::int count from information_schema.tables where table_schema='public' and table_name in ('notifications','incidents','incident_events')",
        ),
      ).toMatchObject({ rows: [{ count: 3 }] });
    } finally {
      await client.close();
  ...
  it("upgrades schema 10 to 11 with reliability daily rollups", async () => {
    const client = createPostgresqlClient(connectionString!);
    try {
      await client.pool.query("drop schema public cascade; create schema public");
      const files = [
      ...
      await executePostgresqlMigration(
        client.pool,
        await readFile(
          new URL("../drizzle/postgresql/0011_typical_joystick.sql", import.meta.url),
          "utf8",
        ),
      );
      expect(
        await client.pool.query("select schema_version from server_settings where id='global'"),
      ).toMatchObject({ rows: [{ schema_version: 11 }] });
      expect(
        await client.pool.query(
          "select count(*)::int count from information_schema.tables where table_schema='public' and table_name=$1",
  ...
  it("upgrades schema 11 to 12 with service_slos", async () => {
    const client = createPostgresqlClient(connectionString!);
    try {
      await client.pool.query("drop schema public cascade; create schema public");
      const files = [
      ...
      await executePostgresqlMigration(
        client.pool,
        await readFile(
          new URL("../drizzle/postgresql/0012_blue_colonel_america.sql", import.meta.url),
          "utf8",
        ),
      );
      expect(
        await client.pool.query("select schema_version from server_settings where id='global'"),
      ).toMatchObject({ rows: [{ schema_version: 12 }] });
      expect(
        await client.pool.query(
          "select count(*)::int count from information_schema.tables where table_schema='public' and table_name=$1",
          ["service_slos"],
  ...
  it("upgrades schema 12 to 13 with service_reliability_hourly", async () => {
    const client = createPostgresqlClient(connectionString!);
    try {
      await client.pool.query("drop schema public cascade; create schema public");
      const files = [
      ...
      await executePostgresqlMigration(
        client.pool,
        await readFile(
          new URL("../drizzle/postgresql/0013_breezy_silver_samurai.sql", import.meta.url),
          "utf8",
        ),
      );
      expect(
        await client.pool.query("select schema_version from server_settings where id='global'"),
      ).toMatchObject({ rows: [{ schema_version: 13 }] });
      expect(
        await client.pool.query(
          "select count(*)::int count from information_schema.tables where table_schema='public' and table_name=$1",
      ...
      await executePostgresqlMigration(
        client.pool,
        await readFile(
          new URL("../drizzle/postgresql/0014_superb_vance_astro.sql", import.meta.url),
          "utf8",
        ),
      );
      expect(
