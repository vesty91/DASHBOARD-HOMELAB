...
import type { JellyfinPermissionsView } from "./types";
...
export function jellyfinPermissionsView(actor: IntegrationActor): JellyfinPermissionsView {
  ...
  return {
    canRead: integrationUse && hasAny(actor, ["jellyfin.read"]),
    canManage: hasAny(actor, ["integration.manage"]),
  };
}

export type JellyfinAccessKind = "read" | "manage";

export function assertJellyfinAccess(actor: IntegrationActor, kind: JellyfinAccessKind): void {
  if (!isActive(actor)) throw new IntegrationError("UNAUTHORIZED", "Authentication required");
  const view = jellyfinPermissionsView(actor);
  switch (kind) {
    case "read":
