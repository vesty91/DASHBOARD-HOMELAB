...
describe("seerr definition", () => {
  it("is a status adapter with bounded request actions and a required API key", () => {
    expect(seerrIntegrationDefinition.id).toBe(SEERR_INTEGRATION_ID);
    expect(seerrIntegrationDefinition.id).toBe("seerr");
    expect(seerrIntegrationDefinition.capabilities).toEqual(["status.read", "requests.manage"]);
