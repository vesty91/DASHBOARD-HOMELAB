export interface SeerrRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const SEERR_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemorySeerrRefreshFence implements SeerrRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = SEERR_REFRESH_FENCE_MAX_ENTRIES) {
