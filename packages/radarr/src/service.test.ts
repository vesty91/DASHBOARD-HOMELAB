...
import { radarrIntegrationDefinition } from "./definition";
import { MemoryRadarrOverviewCoalescer } from "./overview-coalescer";
import { MemoryRadarrRefreshRateLimiter } from "./rate-limiter";
import { MemoryRadarrRefreshFence } from "./refresh-fence";
import { createRadarrService } from "./service";

const INTEGRATION_ID = "11111111-1111-4111-8111-111111111111";
const KEY = Buffer.alloc(32, 9).toString("base64");
const API_KEY = "notareal-radarr-apikey-0123456789";
...
function officialPayloads(request: SecureHttpRequest): SecureHttpResult {
  ...
  if (pathname === "/api/v3/system/status")
    return json({ version: "4.0.14.2939", appName: "Radarr", startupPath: "/opt/Radarr" });
...
function serviceWith(
  ...
  return createRadarrService({
    store: store.store,
    registry: createIntegrationRegistry().register(radarrIntegrationDefinition),
...
describe("createRadarrService", () => {
  it("returns a sanitized overview for a reader", async () => {
    ...
    expect(JSON.stringify(overview)).not.toContain("/opt/Radarr");
  });

  it("requires integration.use and radarr.read together", async () => {
    const service = serviceWith(async (options) => officialPayloads(options));
    await expect(service.getOverview(INTEGRATION_ID, actor(["radarr.read"]))).rejects.toMatchObject(
      {
        code: "FORBIDDEN",
      },
    );
  ...
  it("rate limits refresh", async () => {
    const limiter = new MemoryRadarrRefreshRateLimiter(1, 60_000, () => 1_000);
    const store = createMemoryStore();
    const service = createRadarrService({
      store: store.store,
      registry: createIntegrationRegistry().register(radarrIntegrationDefinition),
      cache: new MemoryIntegrationCache(),
