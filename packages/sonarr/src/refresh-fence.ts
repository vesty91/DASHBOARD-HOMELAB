export interface SonarrRefreshFence {
  current(integrationId: string): number;
  advance(integrationId: string): number;
}

export const SONARR_REFRESH_FENCE_MAX_ENTRIES = 10_000;

export class MemorySonarrRefreshFence implements SonarrRefreshFence {
  readonly #generations = new Map<string, number>();
  readonly #maxEntries: number;

  constructor(maxEntries = SONARR_REFRESH_FENCE_MAX_ENTRIES) {
