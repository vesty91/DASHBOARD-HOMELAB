...
Dashboard self-hosted pour homelab et NAS. Version **1.0.0**.
...
Images GHCR (après publication du tag `v1.0.0`) :

`ghcr.io/vesty91/dashboard-homelab/{web,worker,realtime,migrate}:1.0.0`

Définir `DASHBOARD_IMAGE_PREFIX=ghcr.io/vesty91/dashboard-homelab` et
`APP_VERSION=1.0.0`, puis `docker compose pull && docker compose up`.
