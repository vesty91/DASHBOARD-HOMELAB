...
Dashboard self-hosted pour homelab et NAS. Version **1.0.1**.
...
Images GHCR (après publication du tag `v1.0.1`) :

`ghcr.io/vesty91/dashboard-homelab/{web,worker,realtime,migrate}:1.0.1`

Définir `DASHBOARD_IMAGE_PREFIX=ghcr.io/vesty91/dashboard-homelab` et
`APP_VERSION=1.0.1`, puis `docker compose pull && docker compose up`.
