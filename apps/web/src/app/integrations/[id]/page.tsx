...
export default async function IntegrationDetailPage({
  ...
  try {
    ...
    const [integration, permissions] = await Promise.all([
    ...
    if (integration.type !== "docker") {
      return (
        <PageContainer>
          ...
          <Link className="ui-btn" href={`/integrations/${id}/edit`}>
            Modifier
          </Link>
        </PageContainer>
      );
    ...
    return (
      <PageContainer>
        <PageHeader title={integration.name} description="Docker" />
                ...
                <div className="docker-container-meta">
                  ...
                  {item.ports[0] ? (
                    <span className="ui-muted">
                      {item.ports[0].publicPort ?? item.ports[0].privatePort}/
                      {item.ports[0].protocol}
                    </span>
                  ) : null}
                  {item.recognizedApp?.lifecycleStatus === "legacy" ? (
                    <Badge tone="warning">Legacy</Badge>
