...
describe("jellyfinUserError", () => {
  it("maps transport and auth codes", () => {
    ...
    expect(jellyfinUserError({ code: "DNS_ERROR" })).toBe(
      "Le serveur Jellyfin est injoignable (DNS).",
    );
    expect(jellyfinUserError({ code: "TLS_ERROR" })).toContain("TLS");
    expect(jellyfinUserError({ code: "UNREACHABLE" })).toBe("Le serveur Jellyfin est injoignable.");
